...
if(isset($_POST['submit'])){
   ...
   $apass = mysqli_real_escape_string($conn, $_POST['apassword']);

   $aselect = mysqli_query($conn, "SELECT * FROM `admins` WHERE aemail = '$aemail' AND apassword = '$apass'") or die('query failed');

   if(mysqli_num_rows($aselect) > 0){
      $row = mysqli_fetch_assoc($aselect);
...
<div class="form-container12">
   ...
   <form action="" method="post">
      ...
      <input type="email" name="aemail" placeholder="enter email" class="box12" required>
      <input type="password" name="apassword" placeholder="enter password" class="box12" required>
